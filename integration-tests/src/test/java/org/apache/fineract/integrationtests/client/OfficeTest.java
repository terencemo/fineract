...
public class OfficeTest extends IntegrationTest {
    ...
    /**
     * Whitelist positive test — all accepted column names from the /api/v1/offices
     * response payload must continue to work after the fix.
     *
     * <p>Covers: id, name, nameDecorated, externalId, hierarchy, openingDate
     * (pattern: ^(id|name(Decorated)?|externalId|hierarchy|openingDate)$)
     */
    ...
    void retrieveOffices_withWhitelistedOrderByColumn_succeeds(String column) throws Exception {
        ...
        assertThat(response.isSuccessful())
                .as("GET /offices?orderBy=%s should return 2xx (whitelist over-blocks), got HTTP %d", column, response.code())
                .isTrue();
        assertThat(response.body()).isNotNull();
    }

    /**
     * Null and whitespace orderBy must be treated as absent (no ORDER BY clause)
     * and accepted, consistent with the behaviour documented in ClientSearchTest
     * (testClientSearchOrderByRejectsEmptyAndWhitespace → isTrue()).
     */
    ...
    void retrieveOffices_withBlankOrderBy_isAcceptedAsAbsent(String blank) throws Exception {
        ...
        assertThat(response.isSuccessful())
                .as("Blank orderBy [%s] should be treated as absent, not rejected", blank)
                .isTrue();
    }

    /**
     * Core ZDRES-035 regression — the exact subquery payloads from the Securin
     * advisory that previously bypassed ColumnValidator and executed on the
     * database. All must be rejected with HTTP 403.
     *
     * <p>Payloads included:
     * <ul>
    ...
    @ValueSource(strings = {
            "(SELECT SLEEP(3))",
            "(SELECT SLEEP(10))",
            "(SELECT IF(SUBSTRING(user(),1,4)='root',SLEEP(2),0))",
            "(SELECT SLEEP(300))",
            "(SELECT 1)",
            "(SELECT 1 FROM dual)",
    })
    ...
    void retrieveOffices_zdres035SubqueryPayloads_areRejected(String payload) throws Exception {
        ...
        assertThat(response.isSuccessful())
                .as("ZDRES-035 subquery [%s] must be rejected — whitelist fix not effective", payload)
                .isFalse();
        assertThat(response.code())
                .as("Expected HTTP 403 for payload [%s], got %d", payload, response.code())
                .isEqualTo(403);
    }

    /**
     * Timing regression — verifies that (SELECT SLEEP(3)) is rejected in
     * application code BEFORE the SQL reaches the database.
     *
     * <p>Without the whitelist fix, orderBy input is concatenated into SQL before
     * ColumnValidator runs, so MariaDB executes the subquery and the response
     * takes ≥ 3 seconds. With the fix, the 403 must arrive well under 1 second.
     *
     * <p>The assertion threshold is 2 000 ms — generous for CI variance, but a
     * 3-second SLEEP executing on the database always exceeds it.
     */
    @Test
    @Order(6)
    void retrieveOffices_sleepSubquery_isRejectedBeforeDatabaseExecution() throws Exception {
        ...
        assertThat(response.code())
                .as("Expected HTTP 403 for SLEEP(3) subquery, got %d", response.code())
                .isEqualTo(403);
        assertThat(elapsed)
    ...
    /**
     * Classic SQL injection patterns that ColumnValidator already blocked before
     * the ZDRES-035 fix. The stricter whitelist must continue to reject them —
     * no regression in pre-existing coverage.
     */
    ...
    void retrieveOffices_classicInjectionPayloads_areRejected(String payload) throws Exception {
        var response = fineractClient().offices.retrieveOffices(false, payload, null).execute();
        assertThat(response.isSuccessful())
                .as("Injection payload [%s] must be rejected", payload)
                .isFalse();
        assertThat(response.code()).isEqualTo(403);
    }

    /**
     * Non-whitelisted column names that are not injection payloads — internal
     * schema columns, table-qualified forms, numeric literals, and a
     * percent-encoded subquery (passed as a literal string through retrofit;
     * the server receives the raw percent signs, which do not match any
     * whitelisted column name and are therefore rejected).
     *
     * <p>Note: blank / whitespace-only values are tested separately in
     * {@link #retrieveOffices_withBlankOrderBy_isAcceptedAsAbsent} because the
     * framework treats them as absent rather than as invalid input.
     */
    ...
    void retrieveOffices_nonWhitelistedColumns_areRejected(String payload) throws Exception {
        var response = fineractClient().offices.retrieveOffices(false, payload, null).execute();
        assertThat(response.isSuccessful())
                .as("Non-whitelisted column [%s] must be rejected by the whitelist", payload)
                .isFalse();
        assertThat(response.code()).isEqualTo(403);
    }
}
